...
def main(split, init, args):
    ...
    for fold in range(max_fold):
        ...
        dir = Path(os.path.join('model-calGNN', args.dataset, args.split_type, 'split'+str(split), 
                                'init'+ str(init)))
        dir.mkdir(parents=True, exist_ok=True)
        file_name_pth = dir / (model_name + '.pt')
        file_name_txt = dir / (str(fold)+ '.txt')
        train_gnn(dataset, model, optimizer, file_name_pth, args)
        ...
        print("Calibrate...")
        calibrate_gnn(dataset, model, file_name_pth, file_name_txt, args)
