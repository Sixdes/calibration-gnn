...
def main(split, init, args):
    ...
    for fold in range(max_fold):
        ...
        gcl_loss_func = GCL_Loss(device, gamma=args.gamma)
        optimizer = torch.optim.Adam(list(model.parameters()) + [gcl_loss_func.gamma], lr=lr, weight_decay=args.wdecay)
            ...
            with torch.no_grad():
                ...
                if val_acc >= vacc_mx or val_loss <= vlss_mn:
                    if val_acc >= vacc_mx and val_loss <= vlss_mn:
                        ...
                        b_epoch = i
                        best_gamma = gcl_loss_func.gamma.item()
        ...
        dir = Path(os.path.join('model_gcl-tune', args.dataset, args.split_type, 'split'+str(split), 
                                'init'+ str(init)))
        dir.mkdir(parents=True, exist_ok=True)
        file_name = dir / (model_name + '.pt')
        print(f"Model_name: {model_name}, Best gamma after training: {best_gamma}, best val accuracy: {vacc_mx}")
