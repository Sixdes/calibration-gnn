...
def main(split, init, args):
    ...
    for fold in range(max_fold):
        ...
        loss_lognorm = LogitNormLoss(device, args.temp_eff)
        for i in range(epochs):
            model.train()
            optimizer.zero_grad()
            logits = model(data.x, data.edge_index)
            # loss = criterion(logits[data.train_mask], data.y[data.train_mask]) 
            loss = loss_lognorm(logits[data.train_mask], data.y[data.train_mask]) 
            loss.backward()
            optimizer.step()
        ...
        dir = Path(os.path.join('model_lognorm', args.dataset, args.split_type, 'split'+str(split), 
                                'init'+ str(init)))
