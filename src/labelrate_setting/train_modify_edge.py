...
def main(run, ratio, is_delete, is_add, args):
    # Evaluation
    ...
    dir = Path(os.path.join('model_labelrate/model_modify_edge_all_hidden16', args.dataset, 'labelrate_'+str(args.labelrate), ratio_name))
...
def per_ratio_train(ratio, num_runs, is_delete=False, is_add=False):
    
    val_total_result = {'acc':[], 'nll':[], 'ece':[]}
    test_total_result = {'acc':[], 'nll':[], 'ece':[]}

    for run in range(num_runs):
        val_result, test_result = main(run, ratio, is_delete, is_add, args)
...
if __name__ == '__main__':
    ...
    if args.is_edge_delete:
        for delete_ratio in delete_ratio_list:
            ...
            per_ratio_train(delete_ratio, num_runs, is_delete=True)
    elif args.is_edge_add:
        for add_num in add_num_list:
            print(f'---------------------------training with add {add_num} edges---------------------------')
            per_ratio_train(add_num, num_runs, is_add=True)
