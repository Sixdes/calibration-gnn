...
def calibrate_gnn_rbs(dataset, gnn, args):
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gnn.to(device)
    data = dataset.data.to(device)
    # Uncal
    ece_un, marg_ece_un, nll_un, test_acc_un = cal_eval_model(
    ...
    # RBS
    cal_logits_test, cal_probs_test = RBS(
    ...
    return cal_logits_test, [ece_un, marg_ece_un, nll_un, test_acc_un], \
        [ece, marg_ece, nll, cal_test_acc]
