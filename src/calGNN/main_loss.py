...
def train_gnn(dataset, gnn, optimizer, file_name, args):
...
def calibrate_gnn(dataset, gnn, save_path_pth, save_path_txt, args):
    data = dataset[0]
    ...
    for num_run in tqdm(range(args.num_runs)):
        ...
        gnn.load_state_dict(torch.load(save_path_pth))
