...
def cal_eval_model(gnn, dataset, device, data_name, gnn_name, draw=False):
    data = dataset[0].to(device)
    with torch.no_grad():
        gnn.to(device)
        gnn.eval()
        test_logits = gnn(dataset.data.x, dataset.data.edge_index)[data.test_mask]
        test_labels = data.y[data.test_mask]

    y_pred_test = test_logits.max(1)[1].detach()
    prob_pred_test = F.softmax(test_logits, dim=1)
    test_acc = ((y_pred_test == test_labels).sum() / len(test_labels)).detach().cpu().numpy()
