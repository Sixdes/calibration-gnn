...
def RBS(data, probs, val_logits, val_labels, test_logits, num_bins):
    ...
    def get_rescaled_logits(T_list, logits, bins_mask_list):
        ...
        logits0 = logits / T
        return logits0
    cal_logits_test = get_rescaled_logits(T_list, torch.tensor(test_logits), bins_mask_list_test)
    cal_probs_test = torch.softmax(cal_logits_test, 1)

    return cal_logits_test, cal_probs_test
